...
export const TaskForGod: React.FC = () => {
  ...
  const [todoChecked, setTodoChecked] = useState<boolean[]>(
    defaultState.todoChecked
  )
  const [todoText, setTodoText] = useState<string[]>(defaultState.todoText)
  const [honorSelected, setHonorSelected] = useState<number>(
    defaultState.honorSelected
  )
  ...
  useEffect(() => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ todoChecked, todoText, honorSelected, honorText })
    )
  ...
  return (
    <div>
      <div className="container" ref={captureRef}>
        <div className="row">
          ...
          <div className="col-md-6 mb-4">
            <div className="d-flex align-items-center mb-2">
              ...
              >
                <path
                  d="M20 60 C 20 60, 50 20, 50 20 C 50 20, 80 60, 80 60"
                  stroke="black"
                  strokeWidth="4"
                  fill="none"
                />
                <path
                  d="M35 80 C 35 80, 50 60, 50 60 C 50 60, 65 80, 65 80"
                  stroke="black"
                  strokeWidth="4"
                  fill="none"
                />
        ...
        <button onClick={handleScreenshot} className="btn btn-primary mt-3">
          Capture Screenshot
        </button>
      </div>
    </div>
